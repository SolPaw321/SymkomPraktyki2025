...
# --- User Params --- #
model_type = "3D"  # "2D" currently not supported
...
global_curvature = meshing.create_curvature_sizing_control("global_curvature", min_local=100.0, max_local=1900.0)
meshing.set_scope(global_curvature, part_expression="* !boi", label_expression="*")

# --- Create ring curvature sizing control --- #
ring_curvature = meshing.create_curvature_sizing_control("ring_curvature", min_local=20.0, max_local=100.0)
meshing.set_scope(ring_curvature, part_expression="*fluid-2*", label_expression="wall")

# --- Create body of influence sizing control --- #
boi_curvature = meshing.create_boi_sizing_control("boi_curvature")
meshing.set_scope(boi_curvature, part_expression="boi")
...
meshing.create_surface_mesh_with_size_control(size_control_names="*curvature*")


# --- Create zones from labels and get the ring --- #
meshing.create_zones_from_labels(part_name="fluid-2")
...
# --- Generate prism mesh --- #
meshing.generate_volume_mesh(prism_control, volume_control, part)
